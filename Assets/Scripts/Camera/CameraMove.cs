...
public class CameraMove : MonoBehaviour {
	...
	void Update () {
        ...
        if (isFollowing)
        {
            ...
            if (Input.mousePosition.y < Screen.height / 15 && Input.mousePosition.y >= 0)
            {
                Camera.main.transform.position += new Vector3(0, -1, 0) * CameraConfig._MoveSpeed * Time.deltaTime;
            }
            if (Input.mousePosition.y > Screen.height * 14 / 15 && Input.mousePosition.y <= Screen.height)
            {
                Camera.main.transform.position += new Vector3(0, 1, 0) * CameraConfig._MoveSpeed * Time.deltaTime;
            }
            if (Input.mousePosition.x < Screen.width / 15 && Input.mousePosition.x >= 0)
            {
                Camera.main.transform.position += new Vector3(-1, 0, 0) * CameraConfig._MoveSpeed * Time.deltaTime;
            }
            if (Input.mousePosition.x > Screen.width * 14 / 15 && Input.mousePosition.x <= Screen.width)
            {
                Camera.main.transform.position += new Vector3(1, 0, 0) * CameraConfig._MoveSpeed * Time.deltaTime;
            }
        }
